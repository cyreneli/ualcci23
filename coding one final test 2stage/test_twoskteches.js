  ...
  p.mousePressed = function() {
    if (p.mouseX >= 0 && p.mouseX <= p.width && p.mouseY >= 0 && p.mouseY <= p.height) {
       ...
       // new circle
        ...
        // // 创建一个新的 SharedCircle 实例并添加到数组中
        // sharedCircles.push(new SharedCircle(p.mouseX, p.mouseY, 20));
      }
  };
...
function sketch2(p) {
  ...
  p.setup = function() {
    p.createCanvas(1600, 500, p.WEBGL);
    p.background(100);
  };

  p.draw = function() {
    p.background(100);//must have this 
    // light
    ...
    for (let i = 0; i < circles.length; i++) {
      ...
      p.rotateX(p.PI / 2);
      p.fill(255, 255, 255); // change color
      p.box(circles[i].size, circles[i].size, agentsCounts[i]*10);
      p.pop();
    }
